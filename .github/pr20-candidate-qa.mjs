...
async function mobileResults(cdp, width, height) {
  ...
  const many = await cdp.eval(`(() => {
    ...
    const open=e.card.querySelector('.research-result-open'),os=getComputedStyle(open),or=rect(open)
    return{count:cards.length,name:e.name?.textContent?.trim(),card:rect(e.card),open:or,openVisible:os.display!=='none'&&os.visibility!=='hidden'&&Number(os.opacity)!==0&&or?.width>0&&or?.height>0,nameScroll:e.name?.scrollHeight,nameClient:e.name?.clientHeight,packScroll:e.pack?.scrollHeight,packClient:e.pack?.clientHeight,relationScroll:e.relation?.scrollHeight??0,relationClient:e.relation?.clientHeight??0}
  })()`)
  assert.ok(many.count > 1, `${width}: expected many results`)
  assert.equal(many.openVisible, true, `${width}: quick-view affordance not actually visible`)
  assert.ok(many.open && many.card && many.open.left >= many.card.left - 1 && many.open.right <= many.card.right + 1 && many.open.top >= many.card.top - 1 && many.open.bottom <= many.card.bottom + 1, `${width}: affordance outside card`)
  ...
  await tab(cdp)
  const focus = await cdp.eval(`(() => {const a=document.activeElement,s=getComputedStyle(a),r=a?.getBoundingClientRect?.();return{isCard:a?.classList?.contains('research-result-card')??false,outline:s.outlineStyle,outlineWidth:s.outlineWidth,visible:!!r&&r.width>0&&r.height>0&&r.bottom>0&&r.top<innerHeight}})()`)
  assert.equal(focus.isCard, true, `${width}: Tab did not reach card`)
  assert.equal(focus.visible, true, `${width}: focused card not visible`)
  ...
  const quickView = await cdp.eval(`(() => {const panel=document.querySelector('.research-quick-view'),scroll=document.querySelector('.quick-view-scroll'),ps=getComputedStyle(panel),ss=getComputedStyle(scroll),r=panel.getBoundingClientRect();return{minHeight:ps.minHeight,height:r.height,scrollOverflowY:ss.overflowY,scrollClient:scroll.clientHeight,scrollHeight:scroll.scrollHeight,documentHeight:document.documentElement.scrollHeight,viewportHeight:innerHeight}})()`)
  assert.ok(Math.abs(parseFloat(quickView.minHeight) - height * 0.55) < 2.5, `${width}: quick-view 55vh minimum changed ${quickView.minHeight}`)
  assert.equal(quickView.scrollOverflowY, 'visible', `${width}: quick-view document scroll changed`)
  assert.ok(quickView.documentHeight >= quickView.viewportHeight, `${width}: quick-view document height collapsed`)
  ...
  const one = await cdp.eval(`(() => {
    const results=document.querySelector('.research-results'),scroll=document.querySelector('.research-results-scroll'),list=document.querySelector('.research-results-list'),card=document.querySelector('.research-result-card')
    const rr=results.getBoundingClientRect(),sr=scroll.getBoundingClientRect(),lr=list.getBoundingClientRect(),cr=card.getBoundingClientRect(),rs=getComputedStyle(results),ws=getComputedStyle(document.querySelector('.research-workspace'))
    return{resultsHeight:rr.height,minHeight:rs.minHeight,workspaceAlignSelf:ws.alignSelf,panelAfterScroll:rr.bottom-sr.bottom,scrollAfterList:sr.bottom-lr.bottom,listAfterCard:lr.bottom-cr.bottom,trailingBlank:rr.bottom-cr.bottom,text:card.textContent?.trim(),loadMore:!!document.querySelector('.load-more')}
  })()`)
  assert.ok(parseFloat(one.minHeight) <= 1, `${width}: applied Results min-height still active ${one.minHeight}`)
  assert.equal(one.workspaceAlignSelf, 'start', `${width}: workspace stretch returned`)
  assert.equal(one.loadMore, false, `${width}: unexpected load-more in one-result case`)
  assert.ok(Math.abs(one.panelAfterScroll) <= 2.5, `${width}: panel stretches after results scroll ${one.panelAfterScroll}`)
  assert.ok(Math.abs(one.scrollAfterList) <= 2.5, `${width}: results scroll stretches after list ${one.scrollAfterList}`)
  assert.ok(Math.abs(one.listAfterCard) <= 2.5, `${width}: results list stretches after card ${one.listAfterCard}`)
  assert.ok(one.trailingBlank <= 7.5, `${width}: unexplained internal trailing space ${one.trailingBlank}`)
  ...
  const zero = await cdp.eval(`(() => {const results=document.querySelector('.research-results'),scroll=document.querySelector('.research-results-scroll'),state=document.querySelector('.state-message'),rr=results.getBoundingClientRect(),sr=scroll.getBoundingClientRect(),tr=state.getBoundingClientRect();return{cards:document.querySelectorAll('.research-result-card').length,text:state.textContent?.trim(),resultsHeight:rr.height,panelAfterScroll:rr.bottom-sr.bottom,scrollAfterState:sr.bottom-tr.bottom,minHeight:getComputedStyle(results).minHeight}})()`)
  assert.equal(zero.cards, 0)
  assert.ok(parseFloat(zero.minHeight) <= 1, `${width}: zero-result panel inherited mobile minimum ${zero.minHeight}`)
  assert.ok(Math.abs(zero.panelAfterScroll) <= 2.5, `${width}: zero-result panel stretches after scroll ${zero.panelAfterScroll}`)
  assert.ok(Math.abs(zero.scrollAfterState) <= 2.5, `${width}: zero-result scroll stretches after state ${zero.scrollAfterState}`)
  await cdp.shot(`${prefix}-zero-result.png`)
  return { many, focus, quickView, one, zero }
}
...
async function recipeFlow(cdp) {
  ...
  const list = await cdp.eval(`(() => {const choices=[...document.querySelectorAll('.recipe-detail-grid .choice')],panel=document.querySelector('#mobile-recipe-refine-panel'),inner=panel.querySelector('.research-filter-scroll');return{count:choices.length,labels:choices.map(n=>n.textContent?.trim()),panelOverflow:getComputedStyle(panel).overflowY,innerOverflow:getComputedStyle(inner).overflowY,panelClient:panel.clientHeight,panelScroll:panel.scrollHeight,innerClient:inner.clientHeight,innerScroll:inner.scrollHeight}})()`)
  assert.ok(list.count > 36, `expected full recipe catalog beyond former 36 cap, got ${list.count}`)
  ...
  assert.ok(list.panelClient >= list.panelScroll - 2, 'mobile refinement panel clips document content')
  assert.ok(list.innerClient >= list.innerScroll - 2, 'mobile refinement inner area clips document content')
...
async function boundaryFlow(cdp) {
  ...
  for (const width of [980, 981]) {
    ...
    await cdp.nav(applied())
    await waitCatalog(cdp, true)
    const state = await cdp.eval(`(() => {const entry=document.querySelector('.mobile-refine-entry'),filters=document.querySelector('.research-filters'),scroll=document.querySelector('.research-filter-scroll'),er=entry?.getBoundingClientRect(),fr=filters?.getBoundingClientRect();return{entryDisplay:entry?getComputedStyle(entry).display:null,entryVisible:!!er&&er.width>0&&er.height>0&&getComputedStyle(entry).visibility!=='hidden',entryWidth:er?.width??0,filtersDisplay:filters?getComputedStyle(filters).display:null,filtersVisible:!!fr&&fr.width>0&&fr.height>0&&getComputedStyle(filters).visibility!=='hidden',filtersWidth:fr?.width??0,filterOverflowY:scroll?getComputedStyle(scroll).overflowY:null}})()`)
    if (width === 980) { assert.equal(state.entryVisible, true); assert.equal(state.filtersVisible, false) }
    else { assert.equal(state.entryVisible, false); assert.equal(state.filtersVisible, true); assert.equal(state.filterOverflowY, 'auto') }
  ...
  await cdp.nav(applied())
  await waitCatalog(cdp, true)
  const before = await cdp.eval(`(() => {const n=document.querySelector('.research-filter-scroll'),r=n.getBoundingClientRect();return{top:n.scrollTop,client:n.clientHeight,scroll:n.scrollHeight,win:scrollY,overflowY:getComputedStyle(n).overflowY,x:r.left+r.width/2,y:r.top+Math.min(r.height/2,300)}})()`)
  assert.equal(before.overflowY, 'auto', 'desktop filter rail overflow changed')
...
try {
  ...
  console.log('PR20_CANDIDATE_QA_PASS ' + JSON.stringify({ sourceSha: SOURCE_SHA, browserVersion: version, recipeCount: report.checks.recipe.count, recipeFirst: report.checks.recipe.first, recipeLast: report.checks.recipe.last, resultsMin360: report.checks.mobile360.one.minHeight, oneBlank360: report.checks.mobile360.one.trailingBlank, zeroMin360: report.checks.mobile360.zero.minHeight, resultsMin390: report.checks.mobile390.one.minHeight, oneBlank390: report.checks.mobile390.one.trailingBlank, zeroMin390: report.checks.mobile390.zero.minHeight, quickViewMin360: report.checks.mobile360.quickView.minHeight }))
} catch (error) {
