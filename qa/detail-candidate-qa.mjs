...
async function makePage(width, height, { mockApi = true } = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (!mockApi && isRest && ['GET', 'HEAD'].includes(method)) {
      ...
      const body = method === 'HEAD' ? Buffer.from('') : Buffer.from(await response.arrayBuffer())
...
resetMode()
{
  ...
  await page.focus('#detail-tab-overview')
  const tabsToDisclosure = await tabUntil(page, '.detail-disclosure > summary', 6)
  const disclosureFocus = await focusMetrics(page, '.detail-disclosure > summary')
