...
const apiServer = createServer((req, res) => {
  ...
  if (url.pathname.endsWith('/switch_current_variant_options')) {
    ...
    if (filter?.startsWith('eq.')) { res.end(JSON.stringify(variantsByProduct.get(filter.slice(3)) ?? [])); return }
...
async function launch() {
  ...
  const dir = `/tmp/pr22-switch-qa-${process.pid}`
...
async function desktopFlow(cdp) {
  ...
  await clickSelectorContaining(cdp, '.switch-variant-card', '1 kg')
...
async function mobileFlow(cdp) {
  ...
  await cdp.wait(`[...document.querySelectorAll('.switch-variant-card')].some(n=>n.textContent.includes('1 kg'))`, 'mobile single SKU')
  ...
  const fonts = await cdp.platformFonts('.switch-current-summary')
...
try {
  report.desktop = await desktopFlow(cdp)
  report.mobile = await mobileFlow(cdp)
  assert.equal(writeRequests, 0, 'QA attempted a data/analytics write')
  report.productionWrites = writeRequests
  writeFileSync(`${OUT}/report.json`, JSON.stringify(report, null, 2))
  console.log('PR22_KOREAN_BROWSER_QA_PASS', JSON.stringify({ sourceSha: SOURCE_SHA, browserVersion: version, desktopSummary: report.desktop.roundtripSummary, compareDock: report.desktop.dock, mobile: report.mobile.stored, writes: writeRequests }))
} finally {
  cdp.close(); proc.kill('SIGTERM'); rmSync(dir, { recursive: true, force: true }); await new Promise((resolve) => apiServer.close(resolve))
}
