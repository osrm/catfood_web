...
async function mobileResults(cdp, width, height) {
  ...
  const many = await cdp.eval(`(() => {
    ...
    const open=e.card.querySelector('.research-result-open'),os=getComputedStyle(open)
    return{count:cards.length,name:e.name?.textContent?.trim(),card:rect(e.card),open:rect(open),openVisible:os.display!=='none'&&os.visibility!=='hidden'&&Number(os.opacity)!==0&&rect(open)?.width>0&&rect(open)?.height>0,nameScroll:e.name?.scrollHeight,nameClient:e.name?.clientHeight,packScroll:e.pack?.scrollHeight,packClient:e.pack?.clientHeight,relationScroll:e.relation?.scrollHeight??0,relationClient:e.relation?.clientHeight??0}
  })()`)
  ...
  const quickView = await cdp.eval(`(() => {const panel=document.querySelector('.research-quick-view'),scroll=document.querySelector('.quick-view-scroll'),ps=getComputedStyle(panel),ss=getComputedStyle(scroll),r=panel.getBoundingClientRect();return{minHeight:ps.minHeight,height:r.height,scrollOverflowY:ss.overflowY,scrollClient:scroll.clientHeight,scrollHeight:scroll.scrollHeight}})()`)
  ...
  assert.equal(quickView.scrollOverflowY, 'auto', `${width}: quick-view internal scroll changed`)
...
try {
  ...
  console.log('PR20_CANDIDATE_QA_PASS ' + JSON.stringify({ sourceSha: SOURCE_SHA, browserVersion: version, recipeCount: report.checks.recipe.count, recipeFirst: report.checks.recipe.first, recipeLast: report.checks.recipe.last, resultsMin360: report.checks.mobile360.one.minHeight, oneBlank360: report.checks.mobile360.one.trailingBlank, resultsMin390: report.checks.mobile390.one.minHeight, oneBlank390: report.checks.mobile390.one.trailingBlank, quickViewMin360: report.checks.mobile360.quickView.minHeight }))
} catch (error) {
