...
async function launch(width, height, mobile = true) {
  ...
  const port = 17500 + (process.pid % 200) + index * 20
  const dir = `/tmp/compare-header-qa-${process.pid}-${index}`
...
function stableState(value) { return { currentProductId:value.currentProductId, variantSelection:value.variantSelection, compareIds:value.compareIds, compareOpen:value.compareOpen, compareTab:value.compareTab } }
...
async function trustedPointClick(c, selector, matcher = null) {
  const point = await c.eval(`(()=>{
    ...
    return{x,y,text:node.textContent.trim()}
  })()`)
...
async function addExactCandidates(c) {
  ...
  const chosen = CANDIDATE_NAMES.map((name) => rows.find((row) => row.brand === '내추럴발란스' && row.name === name))
  assert.ok(chosen.every(Boolean), `exact candidate pair missing: ${JSON.stringify(chosen)}`)
...
async function openCompare(c) {
  ...
  await c.wait(`document.querySelector('.compare-mobile-head-grid')`, 'mobile header')
  await sleep(160)
}
async function prepare(c, base) { await enterResults(c, base); const pair = await addExactCandidates(c); await openCompare(c); return pair }

async function headerSnapshot(c) {
  return c.eval(`(()=>{
    ...
    const firstRow=document.querySelector('.compare-mobile-overview-row')
    return{
...
async function keyboardActions(c) {
  ...
  for (let i=0;i<30 && found.length<2;i++) {
    await pressTab(c)
    const snap = await c.eval(`(()=>{const n=document.activeElement,s=n?getComputedStyle(n):null;return{matches:Boolean(n?.matches('.compare-mobile-head-actions button')),text:n?.textContent?.replace(/\\s+/g,' ').trim()??null,focusVisible:Boolean(n?.matches(':focus-visible')),outlineStyle:s?.outlineStyle??null,outlineWidth:s?.outlineWidth??null,scrollY}})()`)
    if (snap.matches) found.push(snap)
  }
  assert.equal(found.length, 2, `both header actions must be reachable by Tab: ${JSON.stringify(found)}`)
...
async function runOne(label, base, width, height, candidateChecks=false) {
  const handle = await launch(width,height,true)
  try {
    const { chosen, stable } = await prepare(handle.c, base)
    await alignForShot(handle.c)
    const metrics = await headerSnapshot(handle.c)
    await handle.c.shot(`${label}-${width}.png`)
    let keyboard = null
    if (candidateChecks) {
      assert.ok(metrics.hit.every((x)=>x.hit), `action center hit-test failed: ${JSON.stringify(metrics.hit)}`)
      assert.ok(metrics.hit.every((x)=>x.height>=44), `action target below 44px: ${JSON.stringify(metrics.hit)}`)
      keyboard = await keyboardActions(handle.c)
    }
    return { chosen, stable, metrics, keyboard, network:await networkSummary(handle.c) }
  } finally { await cleanup(handle) }
}

const baseline360 = await runOne('baseline', BASELINE, 360, 844)
const candidate360 = await runOne('candidate', TARGET, 360, 844, true)
const baseline390 = await runOne('baseline', BASELINE, 390, 900)
const candidate390 = await runOne('candidate', TARGET, 390, 900, true)

const boundaryHandle = await launch(390,900,true)
let boundaries
try {
  await prepare(boundaryHandle.c, TARGET)
  async function at(width,mobile) {
    await boundaryHandle.c.send('Emulation.setDeviceMetricsOverride',{width,height:900,deviceScaleFactor:1,mobile,screenWidth:width,screenHeight:900})
    await boundaryHandle.c.send('Emulation.setTouchEmulationEnabled',{enabled:mobile,maxTouchPoints:mobile?5:1})
    await sleep(180)
    return boundaryHandle.c.eval(`(()=>{const m=document.querySelector('.compare-switch-mobile-overview'),d=document.querySelector('.compare-switch-overview-desktop'),n=document.querySelector('.compare-mobile-product-head.is-candidate > strong'),a=document.querySelector('.compare-mobile-head-actions button');return{width:innerWidth,mobileDisplay:m?getComputedStyle(m).display:null,desktopDisplay:d?getComputedStyle(d).display:null,nameFont:n?getComputedStyle(n).fontSize:null,actionFont:a?getComputedStyle(a).fontSize:null,actionMinHeight:a?getComputedStyle(a).minHeight:null}})()`)
  }
...
} finally { await cleanup(boundaryHandle) }

function delta(base, candidate) {
  return {
    headerHeight: candidate.metrics.headGrid.height-base.metrics.headGrid.height,
    firstRowDocTop: candidate.metrics.firstRow.docTop-base.metrics.firstRow.docTop,
  }
}
const report={productSha:PRODUCT_SHA,baselineUrl:BASELINE,targetUrl:TARGET,baseline360,candidate360,baseline390,candidate390,boundaries,deltas:{w360:delta(baseline360,candidate360),w390:delta(baseline390,candidate390)}}
writeFileSync(`${OUT}/report.json`,JSON.stringify(report,null,2))
console.log('MOBILE_SWITCH_COMPARE_HEADER_TYPOGRAPHY_PASS')
