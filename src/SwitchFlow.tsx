...
export default function SwitchFlow({
  ...
  function renderKeepStep() {
    ...
    const selectedChangeLabels = criteriaLabels(change)
    if (changeBrand) selectedChangeLabels.unshift('다른 브랜드')
    selectedChangeLabels.push(...ingredientAvoidTerms.map((term) => `피함 · ${ingredientLabel(term)}`))
    if (noChangeIntent && selectedChangeLabels.length === 0) selectedChangeLabels.push('특별히 바꿀 점 없음')

    const currentCoreFacts = [
        ...
        <main className="switch-step-main">
          <div className="switch-step-header">
            ...
            <div className="switch-keep-change-summary">
              ...
              <span>{selectedChangeLabels.join(' · ') || '특별히 바꿀 점 없음'}</span>
            </div>
          </div>
          ...
          <section className="switch-current-facts-strip switch-current-facts-summary" aria-label="현재 제품에서 확인된 정보">
            <div>
              <span>현재 제품에서 확인됨</span>
              <strong>{currentCoreFacts.join(' · ')}</strong>
            </div>
            <p>
