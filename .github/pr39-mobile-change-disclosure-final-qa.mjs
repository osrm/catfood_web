...
async function launch(width, height = 844, mobile = width <= 760) {
  ...
  const port = 12500 + (process.pid % 100) + launchNo++ * 40
  const dir = `/tmp/pr39-change-final-${process.pid}-${launchNo}`
...
async function clickExact(c, selector, label) {
  const point = await c.eval(`(()=>{const n=[...document.querySelectorAll(${q(selector)})].find(n=>n.textContent.trim()===${q(label)});if(!n)return null;const r=n.getBoundingClientRect();const s=getComputedStyle(n);const x=Math.max(2,Math.min(innerWidth-2,r.left+r.width/2));const y=Math.max(2,Math.min(innerHeight-2,r.top+r.height/2));const hit=document.elementFromPoint(x,y);if(s.display==='none'||s.visibility==='hidden'||r.bottom<=0||r.top>=innerHeight||r.right<=0||r.left>=innerWidth)return{offscreen:true,rect:{top:r.top,bottom:r.bottom,left:r.left,right:r.right},text:n.textContent.trim()};if(!hit||!n.contains(hit))return{blocked:true,hit:hit?.tagName||null,hitClass:hit?.className||'',rect:{top:r.top,bottom:r.bottom,left:r.left,right:r.right},text:n.textContent.trim()};window.__qaTrustedClick=null;n.addEventListener('click',e=>window.__qaTrustedClick=e.isTrusted,{once:true,capture:true});return{x,y,text:n.textContent.trim()}})()`)
...
async function typeText(c, selector, text) {
  ...
  await c.send('Input.dispatchKeyEvent', { type: 'rawKeyDown', key: 'Control', code: 'ControlLeft', windowsVirtualKeyCode: 17, nativeVirtualKeyCode: 17, modifiers: 2 })
...
async function makeExactVisible(c, selector, label) {
  for (let i = 0; i < 40; i++) {
    const r = await c.eval(`(()=>{const n=[...document.querySelectorAll(${q(selector)})].find(n=>n.textContent.trim()===${q(label)});if(!n)return null;const r=n.getBoundingClientRect();const s=getComputedStyle(n);return{ok:s.display!=='none'&&s.visibility!=='hidden'&&r.top>=8&&r.bottom<=innerHeight-8,top:r.top,bottom:r.bottom}})()`)
...
async function scrollToNext(c) {
  ...
  for (; events < 20; events++) {
    ...
    if (rect && rect.top >= 8 && rect.bottom <= height - 8) break
    await wheel(c, 220)
  }
...
async function basicScenario(kind) {
  ...
  try {
    await enterChange(c)
    const actualViewport = await viewport(c)
    assert.deepEqual({ width: actualViewport.width, height: actualViewport.height }, { width: 360, height: 844 })
    ...
    return { kind, viewport: actualViewport, nextInitial, baselineActualScroll: BASELINE_SCROLL, scrollToAction, network: await networkReport(c) }
  } finally {
...
async function pressKey(c, key, code, keyCode, modifiers = 0, text = '') {
  ...
  await c.send('Input.dispatchKeyEvent', { type: 'rawKeyDown', ...payload, ...(text ? { text, unmodifiedText: text } : {}) })
  await c.send('Input.dispatchKeyEvent', { type: 'keyUp', ...payload })
  await sleep(80)
}

async function pressTab(c, shift = false) {
  await pressKey(c, 'Tab', 'Tab', 9, shift ? 8 : 0)
}
...
async function activateDisclosureKey(c, key, code, keyCode, text = '') {
  await c.eval(`(()=>{const n=document.querySelector('.switch-change-additional-toggle');window.__qaKeyboardClick=null;n.addEventListener('click',e=>window.__qaKeyboardClick=e.isTrusted,{once:true,capture:true})})()`)
  await pressKey(c, key, code, keyCode, 0, text)
  assert.equal(await c.eval('window.__qaKeyboardClick'), true, `${key} did not produce a trusted button click`)
}

async function focusGeometry(c) {
  return c.eval(`(()=>{const n=document.querySelector('.switch-change-additional-toggle');const p=n.parentElement;const r=n.getBoundingClientRect();const pr=p.getBoundingClientRect();const s=getComputedStyle(n);const ps=getComputedStyle(p);const width=parseFloat(s.outlineWidth)||0;const offset=parseFloat(s.outlineOffset)||0;const extent=width+Math.max(0,offset);const clips=ps.overflow!=='visible'&&(r.left-extent<pr.left||r.right+extent>pr.right||r.top-extent<pr.top||r.bottom+extent>pr.bottom);return{active:document.activeElement===n,outlineStyle:s.outlineStyle,outlineWidth:s.outlineWidth,outlineOffset:s.outlineOffset,parentOverflow:ps.overflow,button:{left:r.left,right:r.right,top:r.top,bottom:r.bottom},parent:{left:pr.left,right:pr.right,top:pr.top,bottom:pr.bottom},outlineExtent:extent,clippedByParent:clips}})()`)
}
...
async function advancedScenario() {
  ...
  try {
    ...
    return { viewport: actualViewport, tabCount, focus, selectedState, summary, countCopy, beforeSelectScroll, afterSelectScroll, tabAfterCollapse, reentryState, noChangeCleared: cleared, network: await networkReport(c) }
  } finally {
    await cleanup(handle)
  }
}
...
async function longSummary390() {
  ...
  try {
    await enterChange(c)
    const actualViewport = await viewport(c)
    assert.deepEqual({ width: actualViewport.width, height: actualViewport.height }, { width: 390, height: 900 })
    await makeVisible(c, '.switch-change-additional-toggle')
    await click(c, '.switch-change-additional-toggle')
    ...
    return { viewport: actualViewport, labels, metrics, network: await networkReport(c) }
  } finally {
    await cleanup(handle)
  }
}
...
async function boundary(width) {
  ...
  const handle = await launch(width, height, false)
  const c = handle.c
  try {
    await enterChange(c)
    const actualViewport = await viewport(c)
    assert.deepEqual({ width: actualViewport.width, height: actualViewport.height }, { width, height })
    const result = await c.eval(`(()=>{const mobile=document.querySelector('.switch-change-mobile-criteria');const desktop=document.querySelector('.switch-change-desktop-criteria');const toggle=document.querySelector('.switch-change-additional-toggle');const main=document.querySelector('.switch-step-main');return{mobileDisplay:getComputedStyle(mobile).display,desktopDisplay:getComputedStyle(desktop).display,toggleDisplay:getComputedStyle(toggle).display,toggleExpanded:toggle.getAttribute('aria-expanded'),desktopColumns:getComputedStyle(desktop).gridTemplateColumns,mainOverflowY:getComputedStyle(main).overflowY,documentTop:document.scrollingElement?.scrollTop||0,desktopAdvancedVisible:[...desktop.querySelectorAll('.switch-criterion-section')].every(n=>getComputedStyle(n).display!=='none')}})()`)
    if (width <= 760) {
    ...
    return { viewport: actualViewport, ...result, network: await networkReport(c) }
  } finally {
    await cleanup(handle)
  }
}
...
assert.equal(report.advanced.focus.clippedByParent, false, `focus outline clipped by disclosure parent: ${JSON.stringify(report.advanced.focus)}`)
