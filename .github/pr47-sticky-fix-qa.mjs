...
const BASELINE_BASE = process.env.BASELINE_BASE || 'https://osrm.github.io/catfood_web/'
...
const report = {
  ...
  status: 'running',
  environment: {
    candidateNavigationBase: BASE,
    baselineNavigationBase: BASELINE_BASE,
    candidateKind: 'exact candidate build served from runner localhost /catfood_web/',
    baselineKind: 'deployed GitHub Pages main baseline',
  ...
  baseline: null, visual: [], boundary: null, error: null,
}
const save = () => writeFileSync(OUT + '/report.json', JSON.stringify(report, null, 2))

try {
  report.baseline = await baselineCase(discovery.pair); save()
  for (const dimensions of [[360,844],[390,900]]) {
    const width = dimensions[0], height = dimensions[1]
    report.visual.push(await visualCase('lookup', width, height, discovery.pair, discovery.packages, { ax:false, sticky:true, name:'candidate-lookup-' + width })); save()
    report.visual.push(await visualCase('explore', width, height, discovery.pair, discovery.packages, { ax:false, sticky:true, name:'candidate-explore-' + width })); save()
    report.visual.push(await visualCase('lookup', width, height, discovery.longPair, discovery.packages, { ax:false, sticky:true, name:'candidate-same-brand-' + width })); save()
  }
  report.boundary = await boundary(discovery.pair); save()
  report.status = 'pass'; save()
  console.log('PR47_STICKY_FIX_QA_PASS')
  console.log(JSON.stringify({
    productSha:report.productSha,
    baseline:{url:report.baseline.url,compareStage:report.baseline.compareStage,p1:report.baseline.p1,tableReleaseReachable:report.baseline.tableReleaseReachable},
    candidate:report.visual.map(v=>({url:v.url,mode:v.mode,width:v.width,compareStage:v.sticky.compareStage,p1:v.sticky.p1,p2:v.sticky.p2,tableReleaseReachable:v.sticky.tableReleaseReachable,bodySample:v.sticky.bodySample})),
