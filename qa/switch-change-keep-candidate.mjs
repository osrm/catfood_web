...
async function state(page,step){
  return page.evaluate((step)=>{
    ...
    return {
      ...
      reference:{box:rect(reference),text:norm(reference?.textContent),name:{text:norm(name?.textContent),box:rect(name),style:style(name)},sku:{text:norm(sku?.textContent),box:rect(sku),style:style(sku)}},
      header:{box:rect(root.querySelector('.switch-step-header h1')),style:style(root.querySelector('.switch-step-header h1')),text:norm(root.querySelector('.switch-step-header h1')?.textContent)},
      choices,
      disclosure:{
        ...
        summary:root.querySelector('.switch-change-additional-summary')?norm(root.querySelector('.switch-change-additional-summary')?.textContent):null,
      },
      changeSummary:root.querySelector('.switch-change-selection-summary')?norm(root.querySelector('.switch-change-selection-summary')?.textContent):null,
      facts:root.querySelector('.switch-current-facts-summary')?norm(root.querySelector('.switch-current-facts-summary')?.textContent):null,
      actions:{
        box:rect(actions),
