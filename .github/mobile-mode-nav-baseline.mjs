...
const proc = spawn(chrome, ['--headless=new','--no-sandbox','--disable-dev-shm-usage','--disable-gpu','--disable-cache','--disable-features=OverlayScrollbar','--force-device-scale-factor=1','--window-size=360,844',`--remote-debugging-port=${port}`,`--user-data-dir=${dir}`,'about:blank'], { stdio: 'ignore' })
let c
try {
  ...
  for (const mode of ['lookup', 'explore', 'switch']) {
    const url = `${ORIGIN}?view=workspace&mode=${mode}`
    await c.send('Page.navigate', { url })
    await c.wait(`document.readyState==='complete'&&document.querySelector('.mode-nav')`, `${mode} mode nav`)
    await sleep(600)
    results[mode] = await c.eval(`(()=>{const nav=document.querySelector('.mode-nav');const buttons=[...nav.querySelectorAll('.mode-button')];const n=getComputedStyle(nav);const measure=b=>{const r=b.getBoundingClientRect(),s=getComputedStyle(b),range=document.createRange();range.selectNodeContents(b);const rects=[...range.getClientRects()];const hit=document.elementFromPoint(r.left+r.width/2,r.top+r.height/2);return{text:b.textContent.trim(),className:b.className,ariaCurrent:b.getAttribute('aria-current'),rect:{x:r.x,y:r.y,width:r.width,height:r.height,bottom:r.bottom,right:r.right},computed:{height:s.height,minHeight:s.minHeight,padding:s.padding,whiteSpace:s.whiteSpace},textRectCount:rects.length,pointerHit:hit===b||b.contains(hit)}};const nr=nav.getBoundingClientRect();return{url:location.href,viewport:{innerWidth,innerHeight,clientWidth:document.documentElement.clientWidth,clientHeight:document.documentElement.clientHeight,dpr:devicePixelRatio},nav:{className:nav.className,rect:{x:nr.x,y:nr.y,width:nr.width,height:nr.height},clientWidth:nav.clientWidth,scrollWidth:nav.scrollWidth,clientHeight:nav.clientHeight,scrollHeight:nav.scrollHeight,overflowX:n.overflowX,overflowY:n.overflowY,display:n.display,gridTemplateColumns:n.gridTemplateColumns,gap:n.gap},buttons:buttons.map(measure)}})()`)
