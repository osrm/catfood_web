...
async function progressAudit(cdp) {
  return cdp.eval(`(() => { const progress = document.querySelector('.switch-progress'), items = [...progress.querySelectorAll('li')]; return { scroll: [progress.clientWidth, progress.scrollWidth], items: items.map((item) => { const number = item.querySelector(':scope > span'), label = item.querySelector('strong'), numberRect = number.getBoundingClientRect(), labelRect = label.getBoundingClientRect(), style = getComputedStyle(label), range = document.createRange(); range.selectNodeContents(label); return { text: label.textContent.trim(), current: item.getAttribute('aria-current'), classes: item.className, numRect: [numberRect.left, numberRect.top, numberRect.width, numberRect.height], labelRect: [labelRect.left, labelRect.top, labelRect.width, labelRect.height], numberAbove: numberRect.bottom <= labelRect.top + 1, whiteSpace: style.whiteSpace, textOverflow: style.textOverflow, overflowX: style.overflowX, client: [label.clientWidth, label.clientHeight], scrollSize: [label.scrollWidth, label.scrollHeight], lineBoxes: range.getClientRects().length, fontSize: style.fontSize } }) } })()`)
}
...
async function mobileFlow(width, height, ctx, result) {
  ...
  try {
    ...
    await selectCurrent(cdp, ctx); const skuLabel = await chooseSku(cdp, ctx); result.sku = { label: skuLabel, id: ctx.sku.variant_id }; result.progress = await progressAudit(cdp); assertMobileProgress(result.progress); await cap('06-sku-progress', ['.switch-progress', '.switch-sku-option.is-selected'])
    ...
    result.candidate = await cdp.eval(`(() => { const box = document.querySelector('.switch-inspector-identity'), image = box.querySelector('.switch-inspector-image'), title = box.querySelector('h1'), boxRect = box.getBoundingClientRect(), imageRect = image.getBoundingClientRect(), titleRect = title.getBoundingClientRect(), style = getComputedStyle(title); return { name: title.textContent.trim(), box: [boxRect.left, boxRect.top, boxRect.width, boxRect.height], image: [imageRect.left, imageRect.top, imageRect.width, imageRect.height], title: [titleRect.left, titleRect.top, titleRect.width, titleRect.height], titleBelowImage: titleRect.top >= imageRect.bottom - 1, titleWidthRatio: titleRect.width / boxRect.width, whiteSpace: style.whiteSpace, textOverflow: style.textOverflow, lineClamp: style.webkitLineClamp || null, scroll: [title.clientWidth, title.scrollWidth, title.clientHeight, title.scrollHeight], imageFit: getComputedStyle(image).objectFit, imageNatural: image.tagName === 'IMG' ? [image.naturalWidth, image.naturalHeight] : null } })()`)
    assert.equal(result.candidate.name, LONG_CANDIDATE); assert.equal(result.candidate.titleBelowImage, true); assert.ok(result.candidate.titleWidthRatio > 0.85, 'candidate title does not use full mobile width'); assert.equal(result.candidate.textOverflow, 'clip'); assert.ok(!result.candidate.lineClamp || result.candidate.lineClamp === 'none', 'candidate title line-clamped'); assert.ok(result.candidate.scroll[1] <= result.candidate.scroll[0] + 1, 'candidate title horizontally clipped'); assert.ok(result.candidate.scroll[3] <= result.candidate.scroll[2] + 1, 'candidate title vertically clipped'); assert.ok(result.candidate.image[2] <= 180 && result.candidate.image[3] <= 180, 'candidate image is oversized on mobile')
    result.detailReach = await reachExactButton(cdp, '.switch-candidate-inspector', '상세 보기 →'); await cap('11-long-candidate-inspector', ['.switch-inspector-identity', '.switch-inspector-actions'])
    ...
    result.font = await cdp.platformFonts('.switch-progress li[aria-current="step"] strong'); assert.ok(result.font.some((item) => item.familyName.includes('Noto Sans CJK KR')), `Korean font mismatch ${JSON.stringify(result.font)}`)
...
const report = { productSha: PRODUCT_SHA, qaSha: QA_SHA, status: 'running', browserVersion, koreanFont: execFileSync('fc-match', [':lang=ko'], { encoding: 'utf8' }).trim(), context: { current: ctx.current, sku: ctx.sku, multiBrand: ctx.multiBrand, longCandidate: LONG_CANDIDATE }, expected: { mobileSteps: 'number above full label; exactly one aria-current=step; no horizontal progress scroll', mobileSearch: 'document scroll only; zero/one content-sized; query + same last row + actual return position retained', mobileCandidate: 'image then full-width full product name; exact enabled 상세 보기 → button reachable', boundary: '760 mobile behavior; 761 and 1280 preserve non-mobile nested search scroll and progress layout', state: 'real SKU plus CHANGE/KEEP survive explicit KEEP→CHANGE and CHANGE→SKU roundtrips' }, mobile: [], boundary: [], error: null }
