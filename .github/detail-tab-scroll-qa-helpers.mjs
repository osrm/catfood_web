  ...
  async connect() {
    ...
    this.ws.addEventListener('message', (event) => {
      ...
      if (message.method === 'Network.responseReceived') {
        this.responses.push({ requestId: message.params.requestId, url: message.params.response.url, status: message.params.response.status, mimeType: message.params.response.mimeType, type: message.params.type })
      }
...
export async function launch(width, height, mobile = true) {
  ...
  const dir = `/tmp/catfood-pr29-postdeploy-${width}-${height}-${process.pid}-${sequence}`
