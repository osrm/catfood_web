...
const OUT = 'qa-artifacts/pr47-postdeploy-sticky-360'
...
function compareUrl(mode, products, base = BASE) {
...
async function readStickyGeometry(c) {
  return c.eval("(()=>{const keys=[...document.querySelectorAll('.compare-mobile-two-product-key th')],table=document.querySelector('.compare-mobile-two-product-table'),rect=n=>{const r=n.getBoundingClientRect();return{top:r.top,bottom:r.bottom,left:r.left,right:r.right,width:r.width,height:r.height}},chain=[];for(let n=keys[0];n;n=n.parentElement){const s=getComputedStyle(n);chain.push({tag:n.tagName,id:n.id||'',className:typeof n.className==='string'?n.className:'',position:s.position,top:s.top,overflowX:s.overflowX,overflowY:s.overflowY,clientHeight:n.clientHeight,scrollHeight:n.scrollHeight,clientWidth:n.clientWidth,scrollWidth:n.scrollWidth,rect:rect(n)})}const doc=document.scrollingElement;return{owner:{tag:doc?.tagName||null,scrollTop:doc?.scrollTop||0,clientHeight:doc?.clientHeight||0,scrollHeight:doc?.scrollHeight||0,maxScroll:doc?doc.scrollHeight-doc.clientHeight:0},ths:keys.map(n=>({id:n.id,text:n.textContent.replace(/\\s+/g,' ').trim(),position:getComputedStyle(n).position,computedTop:getComputedStyle(n).top,rect:rect(n)})),table:rect(table),chain}})()")
}

async function wheelUntil(c, target) {
  for (let i = 0; i < 40; i++) {
    const current = await c.eval('document.scrollingElement.scrollTop')
    if (current >= target - 1) return current
    const delta = Math.min(40, target - current)
    await c.send('Input.dispatchMouseEvent', { type:'mouseWheel', x:180, y:500, deltaX:0, deltaY:delta })
    await sleep(80)
  }
  return c.eval('document.scrollingElement.scrollTop')
}

async function sticky(c, prefix, expectSticky = true) {
  ...
  await c.shot(prefix + '-wheel-1.png')
  await wheelUntil(c, target2)
  await sleep(120)
  const p2 = await readStickyGeometry(c)
  await c.shot(prefix + '-wheel-2.png')
  const releaseThreshold = tableBottomDocument - start.ths[0].rect.height
  const tableReleaseReachable = start.owner.maxScroll >= releaseThreshold
  const bodySample = await c.eval("(()=>{const th=document.querySelector('.compare-mobile-two-product-key th'),r=th.getBoundingClientRect(),x=Math.min(innerWidth-5,r.left+r.width/2),y=Math.min(innerHeight-5,r.bottom+12),n=document.elementFromPoint(x,y);return{tag:n?.tagName||null,className:typeof n?.className==='string'?n.className:'',text:(n?.textContent||'').replace(/\\s+/g,' ').trim().slice(0,120)}})()")
  if (expectSticky) {
    for (const [label, point] of [['p1',p1],['p2',p2]]) {
      for (const th of point.ths) {
        assert.equal(th.position, 'sticky', prefix + ' ' + label + ': position')
        assert.equal(th.computedTop, '0px', prefix + ' ' + label + ': computed top')
        assert.ok(Math.abs(th.rect.top) <= 1.5, prefix + ' ' + label + ': th not pinned ' + JSON.stringify(th.rect))
      }
      assert.ok(point.table.bottom > point.ths[0].rect.bottom + 40, prefix + ' ' + label + ': table boundary reached')
    }
...
async function baselineCase(products) {
  ...
  try {
    const url = compareUrl('lookup', products, BASELINE_BASE)
    await h.c.nav(url)
    await waitTwo(h.c)
    const result = await sticky(h.c, 'baseline-live-lookup-360', false)
    return { url, chrome:h.chrome, ...result, network:await network(h.c) }
...
async function visualCase(mode, width, height, products, packages, options) {
  ...
  try {
    ...
    const stickyResult = options.sticky ? await sticky(h.c, prefix, true) : null
...
async function boundary(products) {
  ...
  try {
    ...
    async function at(width, mobile) {
      ...
      await sleep(180)
      const state = await h.c.eval("(()=>{const m=document.querySelector('.compare-mobile-two-product-overview'),d=document.querySelector('.compare-two-product-overview-desktop'),t=document.querySelector('.compare-mobile-two-product-table');const stage=document.querySelector('.compare-stage'),ss=getComputedStyle(stage);return{width:innerWidth,mobileDisplay:m?getComputedStyle(m).display:null,desktopDisplay:d?getComputedStyle(d).display:null,documentWidth:document.documentElement.scrollWidth,tableClient:t?.clientWidth??null,tableScroll:t?.scrollWidth??null,stageOverflowX:ss.overflowX,stageOverflowY:ss.overflowY}})()")
    ...
    assert.equal(at760.stageOverflowX, 'clip')
    assert.equal(at760.stageOverflowY, 'visible')
    assert.equal(at761.mobileDisplay, 'none')
    assert.notEqual(at761.desktopDisplay, 'none')
    assert.equal(at761.stageOverflowX, 'hidden')
    assert.equal(at761.stageOverflowY, 'auto')
...
const report = {
  ...
  environment: {
    navigationBase: BASE,
    kind: 'deployed GitHub Pages postdeploy',
...
try {
  report.visual.push(await visualCase('lookup', 360, 844, discovery.pair, discovery.packages, { ax:false, sticky:true, name:'postdeploy-lookup-360' })); save()
  const result = report.visual[0]
  assert.equal(result.sticky.compareStage.overflowX, 'clip')
  assert.equal(result.sticky.compareStage.overflowY, 'visible')
  assert.equal(result.sticky.start.owner.tag, 'HTML')
  assert.ok(result.metrics.document.scrollWidth <= result.metrics.document.clientWidth + 1)
  assert.ok(result.metrics.table.scrollWidth <= result.metrics.table.clientWidth + 1)
  assert.deepEqual(result.metrics.identities.map((x) => x.name), discovery.pair.map((p) => p.canonical_name))
  assert.ok(result.sticky.bodySample.tag)
  report.status = 'pass'; save()
  console.log('PR47_POSTDEPLOY_STICKY_360_PASS')
  console.log(JSON.stringify({
    mergeSha:report.mergeSha,
    navigationUrl:result.url,
    compareStage:result.sticky.compareStage,
    position1:{scrollTop:result.sticky.p1.owner.scrollTop,ths:result.sticky.p1.ths.map((x)=>({text:x.text,top:x.rect.top,bottom:x.rect.bottom,computedTop:x.computedTop})),tableBottom:result.sticky.p1.table.bottom},
    position2:{scrollTop:result.sticky.p2.owner.scrollTop,ths:result.sticky.p2.ths.map((x)=>({text:x.text,top:x.rect.top,bottom:x.rect.bottom,computedTop:x.computedTop})),tableBottom:result.sticky.p2.table.bottom},
    tableRelease:{threshold:result.sticky.releaseThreshold,maxScroll:result.sticky.start.owner.maxScroll,reachable:result.sticky.tableReleaseReachable},
    horizontal:{document:[result.metrics.document.clientWidth,result.metrics.document.scrollWidth],table:[result.metrics.table.clientWidth,result.metrics.table.scrollWidth]},
    identities:result.metrics.identities.map((x)=>({brand:x.brand,name:x.name})),
    bodySample:result.sticky.bodySample,
