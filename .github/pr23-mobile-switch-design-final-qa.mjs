...
async function searchCount(browser, query) { await setQuery(browser, query); await browser.wait(`document.querySelector('.switch-find-results-heading span')?.textContent.includes('개 표시')`, 'search count label'); const first = await browser.eval(`document.querySelectorAll('.switch-find-result').length`); await sleep(180); const second = await browser.eval(`document.querySelectorAll('.switch-find-result').length`); assert.equal(second, first, `unstable search count: ${query}`); return second }
...
async function mobileSearch(browser, prefix, ctx) {
  ...
  evidence.manyCount = await searchCount(browser, ctx.manyBrand); assert.ok(evidence.manyCount >= 2); evidence.manyQuery = ctx.manyBrand; evidence.list = await browser.eval(`(() => { const list=document.querySelector('.switch-find-results-list'),style=getComputedStyle(list);return{clientHeight:list.clientHeight,scrollHeight:list.scrollHeight,overflowY:style.overflowY,maxHeight:style.maxHeight}})()`); assert.equal(evidence.list.overflowY, 'visible'); assert.equal(evidence.list.maxHeight, 'none'); assert.ok(evidence.list.scrollHeight <= evidence.list.clientHeight + 1)
...
async function mobileFlow(width, height, ctx, result) {
  ...
  try {
    ...
    await clickExact(browser, '.switch-step-actions', '← 바꿀 것 수정'); await waitState(browser, `s.step==='change'`, 'KEEP to CHANGE explicit back'); result.roundtrip.afterKeepBack = await state(browser); for (const snapshot of [result.roundtrip.afterKeepBack]) { assert.equal(snapshot.variantSelection.variantId, ctx.sku.variant_id); assert.equal(snapshot.changeBrand, true); assert.equal(snapshot.keep.feedType, ctx.current.feed_type) } result.captures.push(await capture(browser, prefix, '08-change-after-keep-back'))
...
async function boundaryFlow(width, height, ctx, result) {
  ...
  try {
    await browser.navigate(`${BASE}?view=workspace&mode=switch`); await browser.wait(`document.querySelector('.switch-find-search input')`, 'boundary search'); result.count = await searchCount(browser, ctx.manyBrand); result.search = await browser.eval(`(() => { const list=document.querySelector('.switch-find-results-list'),style=getComputedStyle(list);return{clientHeight:list.clientHeight,scrollHeight:list.scrollHeight,overflowY:style.overflowY,maxHeight:style.maxHeight}})()`)
    if (width === 760) { assert.equal(result.search.overflowY, 'visible'); assert.equal(result.search.maxHeight, 'none'); assert.ok(result.search.scrollHeight <= result.search.clientHeight + 1) } else { assert.equal(result.search.overflowY, 'auto'); assert.notEqual(result.search.maxHeight, 'none'); assert.ok(result.search.scrollHeight > result.search.clientHeight) } result.captures.push(await capture(browser, prefix, 'boundary-search'))
